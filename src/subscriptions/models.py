...
def user_sub_post_save(sender, instance, *args, **kwargs):
    ...
    if not subscription_obj:
        if not ALLOW_CUSTOM_GROUPS:
            ...
            all_subs = Subscription.objects.filter(active=True)
            all_sub_groups = all_subs.values_list('groups__id', flat=True)
            all_sub_groups_set = set(all_sub_groups)
            
            current_groups = user.groups.all().values_list('id', flat=True)
            current_groups_set = set(current_groups)
            
            # Keep only non-subscription groups
            final_groups_ids = list(current_groups_set - all_sub_groups_set)
